...
const Auth = () => {
  ...
  const handleInstallClick = async () => {
    ...
    if (!deferredPrompt) {
      if (isStandalone) {
        toast({ title: t('alreadyInstalled'), description: t('alreadyInstalledDesc') });
      } else if (isIos) {
        toast({
          title: t('installApp'),
        ...
        toast({
          title: t('installNotAvailable'),
          description: t('installNotAvailableDesc'),
        });
      }
      return;
    };
    ...
    if (outcome === 'accepted') {
      toast({ title: t('installationStarted'), description: t('installationStartedDesc') });
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="glass-card p-8">
          ...
          <p className="text-center mt-4 text-muted-foreground">{t('pleaseWait')}</p>
        </div>
      </div>
    );
  }
